C DISHIN.F - Gateway function for H_infinity or H_2 design of
C            discrete-time systems using SLICOT routines
C            SB10DD and SB10ED.
C
...
C
C Matlab call:
C   [AK,BK,CK,DK,(RCOND)] = dishin(task,A,B,C,D,ncon,nmeas,(gamma))
C
C   task = 1 :  [AK,BK,CK,DK,(RCOND)] = dishin(1,A,B,C,D,ncon,nmeas,
C                                              gamma)
C   task = 2 :  [AK,BK,CK,DK,(RCOND)] = dishin(2,A,B,C,D,ncon,nmeas)
C
C Purpose:
C
C   task = 1:
...
C       for Hinf and H2 design of linear discrete-time control systems.
C       Report99-8, Department of Engineering, Leicester University,
C       April 1999.
C
C Contributor:
...
C .. Scalar parameters used by SLICOT subroutines ..
     ...
     $                  LDDK, LDR, LDWORK, LDX, LDZ, M, N, NCON, NMEAS,
     $                  P
...
C .. Local variables and constant dimension arrays ..
      ...
      DOUBLE PRECISION  RCOND(8)
C
C .. External subroutines ..
      EXTERNAL          SB10DD, SB10ED
C
...
C
      IF ( NRHS.LT.7 ) THEN
         CALL mexErrMsgTxt
     $        ( 'DISHIN requires at least 7 input arguments' )
      ELSE IF ( NLHS.LT.4 ) THEN
         CALL mexErrMsgTxt
     $        ( 'DISHIN requires at least 4 output arguments' )
      END IF
C
...
C
      IF ( TASK.EQ.1 ) THEN
         IF ( NRHS.LT.8 ) THEN
            CALL mexErrMsgTxt
     $          ( 'DISHIN requires at least 8 input arguments' )
         END IF
      END IF
C
...
C
      ...
      IF ( TASK.EQ.1 ) THEN
         LIWORK = MAX( 2*MAX ( NCON, N ), M, NCON + NMEAS, N*N )
         LDWORK = MAX( ( N + Q )*( N + Q + 6 ),
...
C
      ...
      IF ( NLHS.GT.4 ) THEN
         ...
         ELSE
            LDR = 7
         END IF
         PLHS(5) = mxCreateDoubleMatrix( LDR, 1, 0 )
         CALL mxCopyReal8ToPtr( RCOND, mxGetPr( PLHS(5) ), LDR )
      END IF
C
C Deallocate local arrays.
C !Fortran 90/95
      IF ( TASK.EQ.1 ) THEN
         DEALLOCATE( A, B, C, D, AK, BK, CK, DK, X, Z, BWORK, IWORK,
     $               DWORK )
      ELSE
...
C
      IF ( INFO.NE.0 ) THEN
         IF ( TASK.EQ.1 ) THEN
            WRITE( TEXT, '('' INFO = '',I4,'' ON EXIT FROM SB10DD'')'
     $           ) INFO
         ELSE
            WRITE( TEXT, '('' INFO = '',I4,'' ON EXIT FROM SB10ED'')'
     $           ) INFO
         END IF
      END IF
C
