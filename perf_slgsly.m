...
for Nt = sz:sz+Ntests10-1,
  ...
  for mc = n/Ntests10 : n/Ntests10 : n
    ...
    A = rand(n,n) - n*eye(n);  E = rand(n,n) + n*eye(n);  
    C  = rand(m,n);
...
%   SLICOT calculations.
    ...
    X = XC'*XC;
    res(l,4) = norm(A'*X*E + E'*X*A + C'*C,'fro')/max(1,norm(X,'fro'));
  end  
end  
...
for Nt = sz2:sz2+Ntests2-1,
  ...
  for mc = n/Ntests2 : n/Ntests2 : n
    ...
    A = rand(n,n)/n - n*eye(n);  E = rand(n,n) + n*eye(n);  
    C  = rand(m,n);
...
for Nt = sz2:sz2+Ntests2-1,
  ...
  for mc = n/Ntests2 : n/Ntests2 : n
    ...
    A = triu(rand(n,n))/n - n*eye(n);  
    E = triu(rand(n,n)) + n*eye(n);   
    C  = rand(m,n);
