function [ z, rnk, infz, Kronl, Kronr, infe, niz, Af, Ef ] = slzero( sys, tol, bal )
%SLZERO  Computes the normal rank, zeros, and the Kronecker
%        structure of the system pencil for a standard or descriptor
%        system, SYS = (A,B,C,D,E), with possibly rectangular
%        matrices A and E.
%
%        Z = SLZERO(SYS)  computes the zeros of the system SYS. 
%        
%        [Z,RNK,INFZ,KRONL,KRONR,INFE,NIZ,AF,EF] = SLZERO(SYS,TOL,BAL)  has 
...
%
%        V. Sima Dec-22-2003.
%        Revised Jul-14-2004; Dec-15-2008; Mar-03-2009.
%

%
ni = nargin;
if ni < 1,  
    error( ['Usage: Z = SLZERO(SYS)', sprintf('\n'), ...
            '       [Z,RNK,INFZ,KRONL,KRONR,INFE,NIZ,AF,EF] = SLZERO(SYS,TOL,BAL)' ] );
end
%
if ni == 1,
   tol = 0;  
...
if isa( sys, 'ss' ),
   ...
   if iscmplx,
      if ~isempty( E ) && ~isequal( E, eye( size( sys.a ) ) ),
         [ zros, betaz, rnk, infz, Kronl, Kronr, infe, niz, Af, Ef ] ...
              = polezeroz( 3, sys.a, sys.b, sys.c, sys.d, sys.e, tol, bal );
      else  
         [ zros, betaz, rnk, infz, Kronl, Kronr, infe, niz, Af, Ef ] ...
              = polezeroz( 3, sys.a, sys.b, sys.c, sys.d, [ ], tol, bal );
      end
      z = zros ./ betaz;
   else
      if ~isempty( E ) && ~isequal( E, eye( size( sys.a ) ) ),
         [ rzeros, izeros, betaz, rnk, infz, Kronl, Kronr, infe, niz, Af, Ef ] ...
              = polezero( 3, sys.a, sys.b, sys.c, sys.d, sys.e, tol, bal );
      else
         [ rzeros, izeros, betaz, rnk, infz, Kronl, Kronr, infe, niz, Af, Ef ] ...
              = polezero( 3, sys.a, sys.b, sys.c, sys.d, [ ], tol, bal );
      end
