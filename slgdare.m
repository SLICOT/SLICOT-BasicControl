function  [X,F,ev,rcond1] = slgdare(A,E,Q,R,B,L,flag)
%SLGDARE  Solve descriptor DARE with generalized Schur method.
%
%           [X,F,EV,RCOND1] = SLGDARE(A,E,Q,R,B,L,FLAG)  computes the
%         symmetric solution X of the discrete-time algebraic Riccati
%         equation
%                                                      -1
%           E'*X*E = A'*X*A - (L + A'*X*B)*(R + B'*X*B)  (L + A'*X*B)'
%                           + Q,                                    (1)         
%
...
%         
%           [X,EV,RCOND1] = SLGDARE(A,E,Q,R,B,L,FLAG)  does not compute
%         the feedback gain F.
%         
%           [X1,X2,EV,RCOND1] = SLGDARE(A,E,Q,R,B,L,FLAG)  computes a
%                                               [ X1 ]
...
%         is nonsingular.)
%         
%           [X,EV,RCOND1] = SLGDARE(A,E,Q,G,FLAG)  computes the solution
%         X of the following equation, equivalent to (1),
%
%                           -1                     -1
%           E'*X*E = Q - L*R  *L' + A1'*X*(I + G*X)  *A1,
%
%                      -1                 -1
%         where G = B*R  *B', A1 = A - B*R  *L'.
%         
%           [X1,X2,EV,RCOND1] = SLGDARE(A,E,Q,G,FLAG)  computes a
...
%
if ni < 4
   error('Usage: X = SLGDARE(A,E,Q,G,FLAG) or X = SLGDARE(A,E,Q,R,B,L,FLAG)')
elseif ni == 4
   withG = 1;
elseif ni == 5
