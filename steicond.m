function [rcnd,sep,ferr,T,U] = steicond( A,C,X,job,flag )
%STEICOND Computes an estimate of the reciprocal of the condition
%        number of a discrete-time Lyapunov (Stein) equation.
% 
%        RCND = STEICOND(A,C,X)  computes an estimate of the reciprocal
%        of the condition number of the Stein equation
%
%           A'*X*A - X = C,                                          (1)
%
%        where C and X are symmetric matrices.
%
%        [RCND,SEP,FERR] = STEICOND(A,C,X,JOB,FLAG)  has additional
...
%
%        [RCND,SEP,FERR,T,U] = STEICOND(A,C,X,3)  also returns the
...
%
%        If FLAG(1) = 1 or JOB = 1, STEICOND solves reduced Lyapunov
...
%
if ni < 3,
    error('Usage: RCND = STEICOND(A,C,X,JOB,FLAG)')
elseif ni == 3,
    job  = 1;
    flag = [0;0;0];
elseif ni == 4,
