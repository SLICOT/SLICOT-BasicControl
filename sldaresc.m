function  [X,F,ev,rcond1,acc] = sldaresc(A,Q,R,B,L,flag)
%SLDARESC Solve DARE with refined Schur method and estimate condition.
%
%         Case I.
%           [X,F,EV,RCOND1] = SLDARESC(A,Q,R,B,L,FLAG)  computes the
...
%         on the related symplectic matrix.
%         
%           [X,EV,RCOND1,ACC] = SLDARESC(A,Q,R,B,L,FLAG)  does not compute
%         the feedback gain F, but computes the vector ACC, containing
%         estimates of the separation, reciprocal condition number and
%         forward error bound.
%         
%           [X1,X2,EV,RCOND1] = SLDARESC(A,Q,R,B,L,FLAG)  computes a
%                                               [ X1 ]
...
%         
%         Case II.
%           [X,EV,RCOND1,ACC] = SLDARESC(A,Q,G,FLAG)  computes the
...
%         
%           [X1,X2,EV,RCOND1] = SLDARESC(A,Q,G,FLAG)  computes a basis
%                                         [ X1 ]
...
%         Default: FLAG = [0,0,1].
%         
%         See also ARESOLC, SLCARESC, SLCAREGS, SLDAREGS, SLDAREGSV
%         
...
%         
%         Comments:
%         1. Matrices A and R must be nonsingular.
