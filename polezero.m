% POLEZERO.F - MEX-function for computing the poles and zeros
%              of a standard or descriptor system, and the Kronecker
%              structure of the system pencil, using SLICOT routines
%              AB08MD, AB08ND, and AG08BD.
%
%   [(rpoles,ipoles(,betap))(,rzeros,izeros,betaz)(,rnk) ...
%                      (,infz,Kronl,Kronr(,infe,niz,Af,Ef))] ...
%                           = polezero(task,A(,B,C,D)(,E)(,tol,bal))
%   [rpoles,ipoles(,betap)] = polezero(0,A(,E))
%   [rnk]                   = polezero(1,A,B,C,D(,tol,bal))
%   [rnk,infz,Kronl,Kronr(,infe,niz,Af,Ef)] = ...
%                           = polezero(2,A,B,C,D(,E)(,tol,bal))
%   [rzeros,izeros,betaz,rnk,infz,Kronl,Kronr(,infe,niz,Af,Ef)] ...
%                           = polezero(3,A,B,C,D(,E)(,tol,bal))
%   [rpoles,ipoles(,betap),rzeros,izeros,betaz,rnk,infz,Kronl, ...
%                                        Kronr(,infe,niz,Af,Ef)] ...
%                           = polezero(4,A,B,C,D(,E)(,tol,bal))
%
...
%   rpoles - (optional) if task = 0 or task = 4, an n-vector containing
%            the real parts of the (alpha part, if E is general, of the)
%            system poles.
%            Otherwise, rpoles must not be specified as an output
%            parameter.
%   ipoles - (optional) if task = 0 or task = 4, an n-vector containing
%            the imaginary parts of the (alpha part, if E is general, of
%            the) system poles.
%            Otherwise, ipoles must not be specified as an output
%            parameter.
...
%            parameter.
%   rzeros - (optional) if task >= 3, a vector containing the real
%            parts of the alpha part of the system zeros.
%            Otherwise, rzeros must not be specified as an output
%            parameter.
%   izeros - (optional) if task >= 3, a vector containing the imaginary
%            parts of the alpha part of the system zeros.
%            Otherwise, izeros must not be specified as an output
%            parameter.
...
%            reduced pencil (see Method). If task >= 3, Af is in a real
%            Schur form.
