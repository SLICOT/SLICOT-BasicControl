function [rcnd,sep,ferr,T,U] = darecond( A,Q,G,X,job,flag )
%DARECOND Computes an estimate of the reciprocal of the condition
%        number of a discrete-time algebraic Riccati equation.
% 
%        RCND = DARECOND(A,Q,G,X)  computes an estimate of the reciprocal
...
%
%        [RCND,SEP,FERR] = DARECOND(A,Q,G,X,JOB,FLAG)  has additional
...
%        matrix Ac, Ac = inv(I_n + G*X)*A (or Ac = A*inv(I_n + X*G),
%        if FLAG(2) = 1), is supplied on entry:
...
%
%        [RCND,SEP,FERR,T,U] = DARECOND(A,Q,G,X,3)  also returns the
...
%
%        If FLAG(1) = 1 or JOB = 1, DARECOND solves reduced Lyapunov
...
%
if ni < 4,
    error('Usage: RCND = DARECOND(A,Q,G,X,JOB,FLAG)')
elseif ni == 4,
    job  = 1;
    flag = [0;0;0];
elseif ni == 5,
