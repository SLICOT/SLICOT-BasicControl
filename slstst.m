function X = slstst(A,C,flag,trans)
%SLSTST  Solve stable Stein equations.
% 
%        X = SLSTST(A,C,FLAG,TRANS)  computes a Cholesky factor X
%        of the unique symmetric positive semi-definite solution
%        op(X)'*op(X) of a stable Stein (discrete-time Lyapunov) equation
%
...
%        Default: TRANS = 0.
%
%        See also LINMEQ, SLDSYL, SLLYAP, SLSTEI, SLSTLY, SLSYLV
%
...
%
%        Comments:
%        Matrix A must be stable, i.e., all eigenvalues must be strictly
%        inside the unit circle centered in the origin.
%
